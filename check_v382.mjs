...
async function readAttendanceExcelFile(){
  ...
  const idxLateReason = findImportColumn(headers, ['延後下班原因','下班原因','非加班']);
  ...
  for(let i=1;i<rows.length;i++){
    ...
    const lateReason = idxLateReason >= 0 ? excelCellText(row[idxLateReason]) : '';
    ...
    if(!grouped.has(key)) grouped.set(key, {employeeNo, employeeName, department, date, checkIn:'', checkOut:'', noOvertime:false, noOvertimeReason:'', rowNos:[]});
    ...
    if(checkOut && /非加班/.test(lateReason)){ item.noOvertime = true; item.noOvertimeReason = lateReason || '非加班'; }
  }
...
window.previewAttendanceExcelImport = async function(){
  ...
  try{
    ...
    $('attendanceImportPreview').innerHTML = table(['狀態','員工編號','Excel姓名','系統姓名','日期','上班','下班','加班判斷','原始列'], res.rows.slice(0,300).map(r=>[r.status,r.employeeNo,r.employeeName||'-',r.matchedName||'-',r.date,r.checkIn||'-',r.checkOut||'-',r.noOvertime?'非加班不計':'可自動判斷',r.rowNos.join(',')]));
    msg('Excel預覽完成');
  }catch(err){
  ...
  try{
    ...
    for(const r of valid){
      const payload = {
        ...
        work_location: (r.department || 'Excel匯入') + (r.noOvertime ? '｜非加班' : ''),
        gps_lat: null,
        gps_lng: null,
        photo_url: null,
        status: r.noOvertime ? 'excel_import_no_overtime' : 'excel_import'
      };
      let q;
      if(r.existsId){
...
function attendanceMarkedNoOvertime(row){
  if(!row) return false;
  const flagText = [row.status, row.work_location, row.note, row.notes, row.remark, row.remarks].map(x=>String(x||'')).join(' ');
  return /非加班/.test(flagText);
}
function calcAutoOvertimeHoursFromCheckout(date, checkOut, row){
  if(attendanceMarkedNoOvertime(row)) return 0;
...
async function getAutoAttendanceOvertimeBreakdown(empId, month, hourlyRate, skipDates){
  ...
  for(const row of (r.data || [])){
    ...
    const h = calcAutoOvertimeHoursFromCheckout(date, row.check_out, row);
    if(h <= 0) continue;
...
function buildAttendanceStatsRows(data){
  ...
  // V38.2：出缺勤統計的「加班總時數」以原始打卡資料為準。
  // 核准加班單只做獨立欄位參考，不併入總加班，避免統計中心與人工核對時重複或混算。
  ...
  for(const a of attInRange){
    ...
    byEmp.get(id).autoOtHours += calcAutoOvertimeHoursFromCheckout(d, a.check_out, a);
  }
  ...
  const rows = Array.from(byEmp.values()).map(r=>{
    ...
    r.totalOtHours = round2(r.autoOtHours);
...
function attendanceStatsSheetRows(rows){
  ...
  const header = ['員工編號','姓名','部門','出勤天數','遲到次數','遲到總分鐘','早退次數','早退總分鐘','缺上班卡','缺下班卡','自動加班時數','核准加班時數','打卡加班總時數','請假總小時','曠職小時',...leaveTypes.map(t=>t+'小時')];
...
function attendanceStatsDetailRows(data){
  ...
  (data.attendance||[]).filter(a=>hrReportInRange(statsDateFromAttendance(a))).forEach(a=>{
    const d = statsDateFromAttendance(a); const late=statsAttendanceLateMinutes(a); const early=statsAttendanceEarlyMinutes(a); const autoOt=calcAutoOvertimeHoursFromCheckout(d,a.check_out,a);
...
window.generateAttendanceStatsCenter = async function(){
  try{
    ...
    $('hrReportPreview').innerHTML = '<h3>V38.2 出缺勤統計總覽｜'+hrReportPeriodLabel()+'</h3>' + table(['員編','姓名','部門','出勤天數','遲到次數','遲到分鐘','早退次數','早退分鐘','打卡加班總時數','請假小時','缺上班卡','缺下班卡'], display);
