...
async function loadEmployees(){
  try{
    ...
    if($('payEmpSelect')) $('payEmpSelect').innerHTML = opts;
...
window.quickSalaryEdit = async function(id){
  ...
  const r = await supabase.from('employees').update({salary: Number(salary || 0)}).eq('id', id);
...
async function loadEmployeeTable(){
  ...
  $('employeeTable').innerHTML = table(
    ['編號','姓名','帳號','密碼','部門','職稱','本薪','工資基礎','勞健保眷屬','狀態','操作'],
    employees.map(e => [
      e.employee_no || '-',
      e.name || '-',
...
async function loadOvertimeTable(){
  ...
  $('overtimeTable').innerHTML = table(['員工','日期','類型','時間','時數','原因','狀態','操作'], (r.data||[]).map(x => [
    ...
    (x.status || 'pending') === 'pending'
      ? `<button onclick="reviewOvertime('${x.id}','approved')">核准</button><button class="btnRed" onclick="reviewOvertime('${x.id}','rejected')">退回</button>`
      : '已處理'
  ]));
}
...
window.fillApprovedOvertimePay = async function(){
  ...
  try{
    const hourlyRate = employeeWageBase(emp) / payrollSettings.monthlyHours;
...
async function buildPayrollPayload(emp, month){
  ...
  const hourlyRate = wageBase / payrollSettings.monthlyHours;
...
async function loadPayrollTable(){let r=await supabase.from('payroll').select('*, employees(employee_no,name)').order('created_at',{ascending:false}).limit(100);if(r.error)return;$('payrollTable').innerHTML=table(['月份','員工','本薪','加班','津貼','扣款','勞健保眷屬','實發'],(r.data||[]).map(p=>[p.payroll_month,(p.employees?.employee_no||'')+' '+(p.employees?.name||''),'$'+money(p.base_salary),'$'+money(p.overtime_pay),'$'+money(p.allowance),'$'+money(p.deductions),'$'+money(+p.labor_insurance+ +p.health_insurance+ +p.dependent_health_insurance),'$'+money(p.net_salary)]))}
