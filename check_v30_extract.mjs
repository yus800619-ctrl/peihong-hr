...
function overtimeRoundByHalfHourThreshold(hours){
  return round2(Math.max(0, num(hours)));
}
function halfHourFloor(hours){ return overtimeRoundByHalfHourThreshold(hours); }
function halfHourRuleText(){ return '加班依實際分鐘精準計算，不再進位整小時。'; }
...
window.submitOvertime = async function(){
  ...
  if(!date || !start || !end || hours <= 0) return msg('請填寫正確加班日期與時間；平日加班從17:00後起算，依實際分鐘精準計算','bad');
