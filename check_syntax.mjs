...
const createClient = (...a)=>({from(){return this},select(){return this},insert(){return this},update(){return this},delete(){return this},eq(){return this},is(){return this},order(){return this},limit(){return this},rpc(){return this},single(){return this}});
...
window.quickSalaryEdit = async function(id){
  ...
  const autoIns = calculateInsuranceFromSalary(updated.salary, updated.health_dependents_count);
...
function calculateInsuranceFromSalary(salary, dependentsCount=0){
  const insuranceBase = num(salary);
  const laborInsured = bracketAmount(insuranceBase, laborBrackets);
  const healthInsured = bracketAmount(insuranceBase, healthBrackets);
  ...
  return { insuranceBase, laborInsured, healthInsured, laborEmployee, healthSelf, depCount, depHealth, healthTotal: healthSelf + depHealth };
}
...
function syncEmployeeInsurance(prefix){
  ...
  const ins = calculateInsuranceFromSalary(emp.salary, emp.health_dependents_count);
  ...
  if(preview){
    preview.innerHTML = `工資基礎：$${money(wageBase)}｜加班時薪：$${money(round2(wageBase / payrollSettings.monthlyHours))}<br>投保基礎只看本薪：$${money(ins.insuranceBase)}｜勞保級距：${money(ins.laborInsured)}，員工勞保：$${money(ins.laborEmployee)}｜健保級距：${money(ins.healthInsured)}，本人健保：$${money(ins.healthSelf)}，眷屬健保：$${money(ins.depHealth)}`;
  }
  return { wageBase, ...ins };
}
...
async function getLeaveDeductionAmount(empId, month, baseSalary){
  ...
  return {amount: Math.round(total*100)/100, details, rows:r.data||[], annualLeaveDaysUsed: round2(annualHours/8)};
}
    ...
    const html = `
      <div class="card">
        ...
        <table>
          ...
          <tr><td>薪轉帳戶</td><td>${emp.bank_name || '-'} / ${emp.bank_account || '-'}</td></tr>
...
async function deleteRow(tableName, id, reloadFn, label){
  ...
  if(r.error) return msg(`${label}刪除失敗：` + r.error.message + '。請確認已執行 V20_必跑SQL.txt', 'bad');
  msg(`${label}已刪除`);
...
window.deleteEmployee = async function(id){
  ...
  for(const tableName of ['payroll','attendance','leave_requests','overtime_requests']){
    ...
    if(r.error) return msg(`刪除員工相關${tableName}資料失敗：` + r.error.message + '。請確認已執行 V20_必跑SQL.txt', 'bad');
  }
  const r = await supabase.from('employees').delete().eq('id', id);
  if(r.error) return msg('員工刪除失敗：' + r.error.message + '。請確認已執行 V20_必跑SQL.txt', 'bad');
  msg('員工與相關紀錄已刪除');
  await loadAdminAll();
}
