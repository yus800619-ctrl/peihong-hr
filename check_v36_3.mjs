...
const createClient = (...args)=>({from:()=>({select:()=>({}), range:()=>({})})});
...
window.updatePayrollInputsAuto = async function(){
  ...
  try{
    ...
    if(tip) tip.textContent = `已自動帶入核准加班：${ot.totalHours} 小時 / $${money(ot.total)}`;
  }catch(err){
...
// ===== V36.3 HR後台報表中心：單一 Excel 多工作表（只讀資料，不改核心邏輯） =====
...
function hrReportAttendanceSheet(data){
  ...
  data.attendance.filter(a=>hrReportInRange(a.date || a.work_date || a.created_at || a.check_in_time || a.check_in)).forEach(a=>rows.push([hrReportDate(a.date||a.work_date||a.created_at||a.check_in_time||a.check_in),hrReportEmpNo(data.empMap,a.employee_id),hrReportEmpName(data.empMap,a.employee_id),hrReportPick(a,['check_in_time','check_in','clock_in']),hrReportPick(a,['check_out_time','check_out','clock_out']),hrReportPick(a,['location','work_location','note']),[hrReportPick(a,['lat','latitude','gps_lat']),hrReportPick(a,['lng','longitude','gps_lng'])].filter(Boolean).join(','),hrReportPick(a,['photo_url','photo']),hrReportDate(a.created_at)]));
  return rows;
}
...
window.previewHrReportCounts = async function(){
  try{ initHrReportFilters(); const d = await hrReportLoadAllData();
    $('hrReportPreview').innerHTML = table(['資料表','總筆數'], [['員工',d.employees.length],['薪資',d.payroll.length],['打卡',d.attendance.length],['請假',d.leaveReq.length],['加班',d.overtime.length]]);
    msg('報表筆數預覽完成。');
